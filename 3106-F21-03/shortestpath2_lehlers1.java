 ...
 */
public class shortestpath2_lehlers1 {
    ...
    private static void solveProblem() {
        while (true) {
            ...
            for (int i = 0; i < numberOfNodes; i++) {
                nodes[i] = new Node(new ArrayList<>(), Long.MAX_VALUE);
            }
            ...
            for (int edge = 0; edge < numberOfEdges; edge++) {
                nodes[kattio.getInt()].adjacency.add(
                        new Edge(kattio.getInt(), kattio.getInt(), kattio.getInt(), kattio.getInt()));
            }
            dijkstra(startIndex);
            for (int query = 0; query < numberOfQueries; query++) {
                int nodeIndex = kattio.getInt();
                long distance = nodes[nodeIndex].distance;
                kattio.println(distance == Long.MAX_VALUE ? "Impossible" : distance);
            }
            kattio.println();
        }
    }
    ...
    private static void dijkstra(final int startIndex) {
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            for (Edge edge : node.adjacency) {
                long newDistance;
                if (edge.p == 0) {
                    if (edge.t0 > node.distance) {
                        newDistance = edge.t0 + edge.distance;
                    } else {
                        continue;
                    }
                } else {
                    newDistance = (long) Math.ceil(
                            Math.max(0, node.distance - edge.t0) / (double) edge.p) * edge.p + edge.t0 + edge.distance;
                }
                if (newDistance < nodes[edge.adjacentNode].distance) {
                    nodes[edge.adjacentNode].distance = newDistance;
    ...
    static class Node implements Comparable<Node> {
        ...
        public Node(final List<Edge> adjacency, final long distance) {
    ...
    static class Edge {
        ...
        public Edge(final int adjacentNode, final int t0, final int p, final long distance) {
